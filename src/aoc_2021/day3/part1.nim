...
type bitCounter = array[12, int]

proc `+=`(a: var bitCounter, b: bitCounter) =
  for i in 0 ..< len(a):
    a[i] += b[i]

proc runPart1*() =
  var bitCounts: bitCounter
  var numLines = 0

  let inputFile = currentSourcePath().parentDir() / "input.txt"

  for i, line in enumerate(inputFile.lines()):
    let chars: seq[string] = line.strip().split()
    assert len(chars) == len(bitCounter)

    for i, charStr in pairs(chars):
      let parsed = parseInt(charStr)
      assert parsed in {0, 1}
      bitCounts[i] += parsed

    numLines = i
  
  # At each position, the most common int 